...
Effective Date: 27 July 2026SPDX-License-Identifier: LicenseRef-TheDevorger-UPSL-1.3

Copyright © 2026–present TheDevorger.All Rights Reserved.
...
view the Covered Materials;

download one working copy of each lawfully supplied Covered Material for Personal Use;

create a reasonable number of private backup copies;

install Ałek’ryŧhæ Core on Authorized Devices;

retain lawfully supplied .alek Application Packages on Authorized Devices;

associate .alek Application Packages with a compatible installed copy of the Core;

import, open, load, validate, and execute .alek Applications locally through a compatible version of the Core;

execute other Software locally on Authorized Devices where such execution is expressly supported;

compile source code locally where source code is provided;

study source code, manifests, package definitions, and documentation privately for personal education;

test Core and .alek Application functionality privately;

evaluate the Covered Materials for possible acquisition of a separate commercial license;

make private local Modifications solely for personal learning, debugging, accessibility, compatibility testing, or experimentation, subject to Section 11; and

create independent user content through the Software as permitted by Section 20.
...
use remains strictly Personal Use;

use occurs only on Authorized Devices;

the Core and every .alek Application used by You were lawfully obtained;

.alek Applications are opened only through authorized Core functionality unless the Licensor expressly authorizes another method;

no Commercial Use occurs;

no Production Use occurs;

no Distribution occurs;

no public hosting occurs;

no service is provided to another person;

no unauthorized loader, host, launcher, runtime, compatibility layer, converter, unpacker, or substitute is created or used;

no AI System is trained, developed, evaluated, or improved;

no prohibited Automated Processing occurs;

no security, validation, signature, integrity, compatibility, licensing, or technological protection is circumvented;

all legal and proprietary notices remain intact;

Third-Party Materials are used in compliance with their own licenses;

no false claim of ownership or authorship is made;

no false affiliation or endorsement is implied;

no modified Core, .alek Application, or .alek Application Package is represented as official;

no use prohibited elsewhere in this License occurs.
...
use the Covered Materials commercially;

use the Covered Materials for paid work;

use the Covered Materials for unpaid professional work;

use the Covered Materials for a client;

use the Covered Materials for an employer;

use the Covered Materials within a company;

use the Covered Materials within an organization;

use the Covered Materials within an educational institution;

use the Covered Materials within a nonprofit organization;

use the Covered Materials within a government body;

use the Covered Materials for internal business operations;

use the Covered Materials for freelancing;

use the Covered Materials for consulting;

use the Covered Materials in a professional portfolio;

use the Covered Materials in recruitment;

use the Covered Materials in an investor presentation;

use the Covered Materials in a customer demonstration;

use the Covered Materials in a client demonstration;

use the Covered Materials in an enterprise evaluation;

use the Covered Materials in commercial research;

use the Covered Materials to reduce organizational costs;

use the Covered Materials to replace a paid product or service;

use the Covered Materials in advertising;

use the Covered Materials in sponsorship;

use the Covered Materials in promotion;

monetize the Covered Materials;

charge for access to the Covered Materials;

sell services based on the Covered Materials;

provide installation or customization services involving the Covered Materials;

operate a hosted or SaaS deployment;

deploy the Covered Materials in production;

bundle the Covered Materials with another product; or

otherwise obtain financial, professional, competitive, reputational, or organizational benefit from the Covered Materials.
...
normal functionality performed by the Hosting Platform under rights directly granted to it;

local build tools strictly necessary for permitted Personal Use; or

activity that mandatory law expressly permits and does not allow to be contractually restricted.
...
mandatory applicable law expressly permits;

cannot legally be restricted by contract; and

is performed strictly within the limits of that mandatory exception.
...
stop using the Covered Materials;

stop executing the Software;

stop compiling the Software;

stop accessing hosted functionality where access depends on this License;

delete downloaded copies;

delete modified copies;

delete private backup copies unless retention is legally required;

remove unauthorized hosted copies;

remove unauthorized distributed copies;

stop Commercial Use;

stop Production Use;

stop using the Licensor’s branding;

stop AI and Automated Processing;

delete unauthorized datasets;

delete unauthorized embeddings;

delete unauthorized indexes;

delete unauthorized model inputs;

delete unauthorized retrieval databases; and

delete unauthorized .alek Application Packages, converted packages, alternative loaders, launchers, runtimes, compatibility layers, and extracted Core or application components; and

cease every activity relying upon a terminated permission.
...
the affected provision shall be enforced to the maximum lawful extent;

a court may modify the provision only to the minimum extent necessary to make it enforceable; and

the modified provision should preserve the intended protection as closely as legally possible.
...
THEDEVORGER UNIVERSAL PROPRIETARY SOFTWARE LICENSEVersion 1.3
